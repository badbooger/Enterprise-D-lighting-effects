...
>
> **Fuse + barrel jack for single-power-source builds (planning, 2026-08-28, not yet built):** external supply enters through a barrel jack outside the model, in series with an inline fuse (slow-blow, sized above the combined nominal draw — Bridge ~353mA + EngRoom ~133mA, see LED power budget below) and a basic reverse-polarity diode. All battery packs get unplugged when running this way — nothing in this design tolerates a battery pack and external supply both connected at once. Exact fuse rating, diode part, and jack size are intentionally left to the builder, stated as a build-your-own-judgment call in `README.md` rather than a specified BOM item — every build's access points and cable runs differ too much for one prescribed part to fit all of them.
>
> **Kit external-power fuse — 2.5A slow-blow (ordered 2026-09-06):** Separate item from the single-power-source mod's fuse above — this one is for the standard kit's optional 9V power path (wall adapter and/or a 9V USB-C PD breakout/trigger board under evaluation), feeding the battery-input JST directly. Confirmed **not** routed through the ESP32-S3's own USB-C port, which is sized for 5V and would risk damaging the regulator/USB PHY at 9V. 2.5A is well above normal draw (see LED power budget below — well under 1A per section) so it won't nuisance-trip on boot inrush, but still clears a real wiring fault/short before anything overheats. **Ships loose in the kit, not pre-installed** — buyer wires it into the external power lead themselves; recommended placement is inline with the wall/USB-C power outside the model so a blown fuse is a quick swap rather than a teardown. Needs a line in the kit's box-insert warning/instruction card (not yet designed — see DEVNOTES Fix Queue Good-to-have #10).
