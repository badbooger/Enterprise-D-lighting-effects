...
// ── Bridge-AP join + fallback AP ─────────────────────────────────────────────
// The kit has no home-network join at all (control_html.h has no WiFi-join UI
// on purpose) — Bridge always hosts its own fixed AP, so joining it is the
// only path EngRoom has to OTA/mDNS reachability. Without this, EngRoom's OTA
// is unreachable in real kit use (the legacy push-based StartWiFi()/wifi_auto
// flow below is never triggered by anything in the kit UI).
#define BRIDGE_AP_SSID         "Enterprise D"
#define BRIDGE_AP_PASS         "ncc1701-d"
#define BRIDGE_JOIN_TIMEOUT_MS 180000UL   // 3 min, per UTILITY_BATCH_PROVISIONING.md
#define FALLBACK_AP_SSID       "EngRoom Fallback"
#define FALLBACK_AP_PASS       "ncc1701-d"   // same password as Bridge's AP, on purpose

uint32_t   bridgeJoinStartMs = 0;
bool       bridgeStaWasUp    = false;
bool       fallbackApActive  = false;
uint32_t   fallbackStartMs   = 0;
WebServer  fallbackServer(80);

uint8_t broadcastAddress[]  = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00};  // DataPad — run MAC_address_retriver on your board (or use SETMAC below)
uint8_t bridgeAddress[]     = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00};  // Bridge — run MAC_address_retriver on your board (or use SETMAC below)
uint8_t dataPad2Address[]   = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00};  // DataPad 2.8" pocket remote — run MAC_address_retriver on your board (or use SETMAC below)
...
#define PIN_NAC_L      8
#define PIN_PHOTON     4
#define PIN_PHOTON_AFT 9   // EngRoom Photon 2x variant — active-low, no transistor (see HARDWARE.md)
...
#define IDX_PHOTON_AFT 11  // GPIO 9 (Photon 2x variant)
...
// Mirrors Bridge_ESP.ino's pairing window — see its comment for the full flow.
...
bool     phtAftCharging = false;
bool     phtAftFiring   = false;
uint32_t phtAftStartMs  = 0;
...
void pinWrite(int pin, bool on) {
  if (pin == PIN_PHOTON_AFT) {
    ledcWrite(pin, on ? 0 : 255);   // active-low
    return;
  }
...
// Independent of updatePhotonTorpedo() — separate fire control for the aft
// LED (Photon 2x variant). Active-low, so duty is inverted throughout.
void updatePhotonTorpedoAft() {
  uint32_t now = millis();
  if (phtAftCharging) {
    uint32_t elapsed = now - phtAftStartMs;
    if (elapsed >= PHOTON_CHARGE_MS) {
      ledcWrite(PIN_PHOTON_AFT, 0);
      phtAftCharging = false; phtAftFiring = true; phtAftStartMs = now; return;
    }
    ledcWrite(PIN_PHOTON_AFT, 255 - (uint8_t)(255.0f * elapsed / PHOTON_CHARGE_MS));
    return;
  }
  if (phtAftFiring) {
    uint32_t elapsed = now - phtAftStartMs;
    if (elapsed >= PHOTON_FIRE_MS) { ledcWrite(PIN_PHOTON_AFT, 255); phtAftFiring = false; return; }
    ledcWrite(PIN_PHOTON_AFT, 255 - (uint8_t)(255.0f * (1.0f - elapsed / (float)PHOTON_FIRE_MS)));
  }
}
...
void handleLedCommand(int grp, int cmd, int val) {
  ...
  switch (cmd) {
    ...
    case LED_ON:
      ...
      } else if (grp == IDX_PHOTON_AFT) {
        phtAftFiring = false; phtAftCharging = true; phtAftStartMs = millis();
    ...
    case LED_OFF:
      if (grp == GRP_ALL || grp < 0) {
        ...
        phtAftCharging = false; phtAftFiring = false; ledcWrite(PIN_PHOTON_AFT, 255);
      } else if (grp >= 0 && grp < NUM_WINDOWS) {
...
void OnDataRecv(const esp_now_recv_info_t *info, const uint8_t *incomingData, int len) {
  ...
  if (myData.startWifi == 4) {
    ...
    if (pairWindowOpen) {
      ...
      memcpy(broadcastAddress, newMac, 6);
      ...
      Serial.printf("[pair] adopted new DataPad MAC %02X:%02X:%02X:%02X:%02X:%02X — saved to NVS, replied with EngRoom's identity\n",
...
void setup() {
  ...
  pinMode(PIN_PHOTON_AFT, OUTPUT); digitalWrite(PIN_PHOTON_AFT, HIGH);  // active-low, idle off
  ...
  ledcAttach(PIN_PHOTON_AFT, LEDC_FREQ, LEDC_BITS);
  ledcWrite(PIN_PHOTON_AFT, 255);
  ...
  if (prefs.getBytesLength("mac_bridge") == 6) {
    prefs.getBytes("mac_bridge", bridgeAddress, 6);
    Serial.printf("[NVS] Bridge MAC loaded from NVS: %02X:%02X:%02X:%02X:%02X:%02X\n",
      bridgeAddress[0], bridgeAddress[1], bridgeAddress[2],
      bridgeAddress[3], bridgeAddress[4], bridgeAddress[5]);
  } else {
    Serial.printf("[NVS] Bridge MAC: NVS empty, using hardcoded fallback %02X:%02X:%02X:%02X:%02X:%02X\n",
      bridgeAddress[0], bridgeAddress[1], bridgeAddress[2],
      bridgeAddress[3], bridgeAddress[4], bridgeAddress[5]);
  }
  if (prefs.getBytesLength("mac_datapad") == 6) {
    prefs.getBytes("mac_datapad", broadcastAddress, 6);
  ...
  if (prefs.getBytesLength("mac_datapad2") == 6) {
    prefs.getBytes("mac_datapad2", dataPad2Address, 6);
    Serial.printf("[NVS] DataPad2 MAC loaded from NVS: %02X:%02X:%02X:%02X:%02X:%02X\n",
      dataPad2Address[0], dataPad2Address[1], dataPad2Address[2],
      dataPad2Address[3], dataPad2Address[4], dataPad2Address[5]);
  } else {
    Serial.printf("[NVS] DataPad2 MAC: NVS empty, using hardcoded fallback %02X:%02X:%02X:%02X:%02X:%02X\n",
      dataPad2Address[0], dataPad2Address[1], dataPad2Address[2],
      dataPad2Address[3], dataPad2Address[4], dataPad2Address[5]);
  }
  prefs.end();
  ...
  // Kick off a non-blocking join to Bridge's own fixed AP — see note above.
  // ESP32 keeps retrying this in the background on its own (auto-reconnect),
  // so no manual retry loop is needed; updateBridgeJoin() just watches status.
  WiFi.begin(BRIDGE_AP_SSID, BRIDGE_AP_PASS);
  bridgeJoinStartMs = millis();
...
// TEMPORARY — bench test only, remove before any real use. EngRoom has no
// web server this session (see plan), so a test MAC is written over Serial
// instead: "SETMAC bridge AA:BB:CC:DD:EE:FF" / "SETMAC datapad ..." / "SETMAC datapad2 ...".
void handleSerialSetMac() {
  String line = Serial.readStringUntil('\n');
  line.trim();
  if (!line.startsWith("SETMAC ")) return;

  int sp1 = line.indexOf(' ', 7);
  if (sp1 < 0) { Serial.println("[SETMAC] usage: SETMAC <bridge|datapad|datapad2> AA:BB:CC:DD:EE:FF"); return; }
  String peer  = line.substring(7, sp1);
  String macStr = line.substring(sp1 + 1);

  uint8_t parsed[6];
  if (sscanf(macStr.c_str(), "%hhx:%hhx:%hhx:%hhx:%hhx:%hhx",
             &parsed[0], &parsed[1], &parsed[2], &parsed[3], &parsed[4], &parsed[5]) != 6) {
    Serial.println("[SETMAC] bad mac format, expected AA:BB:CC:DD:EE:FF");
    return;
  }

  const char *key;
  uint8_t *target;
  if      (peer == "bridge")   { key = "mac_bridge";   target = bridgeAddress; }
  else if (peer == "datapad")  { key = "mac_datapad";  target = broadcastAddress; }
  else if (peer == "datapad2") { key = "mac_datapad2"; target = dataPad2Address; }
  else { Serial.println("[SETMAC] unknown peer, expected bridge, datapad, or datapad2"); return; }
...
void loop() {
  if (Serial.available()) handleSerialSetMac();
...
// BRIDGE-AP JOIN + FALLBACK AP
// ═══════════════════════════════════════════════════════════════════════════════
// "Oh shit" page only — no theming, matches the EngRoom-fallback-AP precedent
// used elsewhere in this project (Batch_Bootstrap.ino, control_html.h notes).

void handleFallbackRoot() {
  String html = "<html><body><h3>EngRoom Fallback</h3>";
  html += "<p>STA: " + String(WiFi.status() == WL_CONNECTED ? "connected to Bridge" : "retrying Bridge AP") + "</p>";
  html += "<p>MAC: " + WiFi.macAddress() + "</p>";
  html += "<p>Fallback uptime: " + String((millis() - fallbackStartMs) / 1000) + "s</p>";
  html += "<p>Free heap: " + String(ESP.getFreeHeap()) + " bytes</p>";
  html += "<p>Last ESP-NOW receive: ";
  if (!espNowReceived) html += "never";
  else html += String((millis() - lastContactMs) / 1000) + "s ago" + String(connectionLost ? " (marked lost)" : "");
  html += "</p>";
  html += "<p>Last ESP-NOW send: ";
  if (!lastSendKnown) html += "never";
  else html += String(lastSendOk ? "OK" : "FAILED") + ", " + String((millis() - lastSendMs) / 1000) + "s ago";
  html += "</p>";
  html += "<p><a href=\"/ledtest\">Test LEDs (assembly-mode walk)</a></p>";
  html += "<p><a href=\"/allon\">All LEDs On</a> &nbsp; <a href=\"/alloff\">All LEDs Off</a></p>";
  html += "<p><a href=\"/retryjoin\">Retry Bridge Join Now</a></p>";
  html += "<p><a href=\"/reboot\" onclick=\"return confirm('Reboot EngRoom now? Settings are not changed.')\">Reboot</a></p>";
  html += "<p><a href=\"/resetdefault\" onclick=\"return confirm('Reset EngRoom NVS settings to defaults and reboot?')\">Reset to Default</a></p>";
  html += "</body></html>";
  fallbackServer.send(200, "text/html", html);
}

void handleFallbackLedTest() {
  Serial.println("[fallback] LED test (assembly walk) requested");
  asmMode = true; asmPhase = 0; asmWalkIdx = 0; asmStartMs = millis();
  fallbackServer.send(200, "text/plain", "Assembly-mode LED walk started.");
}

// Bridge's own group index for nav lights — board-relative, not a shared
// constant (same "board-relative group number" pattern control_html.h already
// documents for PHOTON/DEFLECTOR/etc.). Must match Bridge_ESP.ino's IDX_NAV.
#define BRIDGE_IDX_NAV 15

void handleFallbackAllOn() {
  Serial.println("[fallback] All LEDs on requested");
  asmMode = false;   // stop the walk fighting a steady all-on state
  for (int i = 0; i < NUM_ALL_PINS; i++) pinWrite(ALL_PINS[i], true);
  // Also reaches out to Bridge over ESP-NOW (Bridge has no "all on" command,
  // so nav is the existing, already-tested stand-in) — doubles as a live
  // send-connectivity check independent of the STA/AP join this page exists for.
  struct_message msg; memset(&msg, 0, sizeof(msg));
  msg.ledGroup = BRIDGE_IDX_NAV; msg.ledCmd = LED_ON; msg.ledValue = 0;
  esp_now_send(bridgeAddress, (uint8_t *)&msg, sizeof(msg));
  fallbackServer.send(200, "text/plain", "All LEDs on (EngRoom) + Bridge nav on.");
}

void handleFallbackAllOff() {
  Serial.println("[fallback] All LEDs off requested");
  asmMode = false;
  allOff();
  // LED_ALL_OFF is a shared command both boards already handle identically —
  // no Bridge-side change needed to also clear Bridge's own LEDs from here.
  struct_message msg; memset(&msg, 0, sizeof(msg));
  msg.ledCmd = LED_ALL_OFF;
  esp_now_send(bridgeAddress, (uint8_t *)&msg, sizeof(msg));
  fallbackServer.send(200, "text/plain", "All LEDs off (EngRoom + Bridge).");
}

void handleFallbackRetryJoin() {
  Serial.println("[fallback] Retry Bridge join requested");
  fallbackServer.send(200, "text/plain", "Retrying Bridge join now...");
  WiFi.disconnect();
  WiFi.begin(BRIDGE_AP_SSID, BRIDGE_AP_PASS);
  bridgeJoinStartMs = millis();
}

void handleFallbackReboot() {
  Serial.println("[fallback] Reboot requested (settings unchanged)");
  fallbackServer.send(200, "text/plain", "Rebooting (settings unchanged)...");
  delay(300);
  ESP.restart();
}

void handleFallbackReset() {
  fallbackServer.send(200, "text/plain", "Resetting settings to defaults (peer pairing preserved), rebooting...");
  // mac_bridge/mac_datapad/mac_datapad2 are factory/USB-provisioned only — no
  // broadcast auto-adopt exists for Bridge<->EngRoom pairing (unlike DataPad's
  // LED_PAIR_DATAPAD). A plain prefs.clear() would wipe them with no recovery
  // path except USB, defeating the point of a no-USB-needed escape hatch.
  prefs.begin("engroom", false);
  uint8_t macBridge[6], macDatapad[6], macDatapad2[6];
  bool hasBridge   = prefs.getBytesLength("mac_bridge")   == 6 && prefs.getBytes("mac_bridge",   macBridge,   6) == 6;
  bool hasDatapad  = prefs.getBytesLength("mac_datapad")  == 6 && prefs.getBytes("mac_datapad",  macDatapad,  6) == 6;
  bool hasDatapad2 = prefs.getBytesLength("mac_datapad2") == 6 && prefs.getBytes("mac_datapad2", macDatapad2, 6) == 6;
  prefs.clear();
  if (hasBridge)   prefs.putBytes("mac_bridge",   macBridge,   6);
  if (hasDatapad)  prefs.putBytes("mac_datapad",  macDatapad,  6);
  if (hasDatapad2) prefs.putBytes("mac_datapad2", macDatapad2, 6);
  prefs.end();
  delay(300);
  ESP.restart();
}
...
void startFallbackAp() {
  ...
  WiFi.mode(WIFI_AP_STA);   // STA side keeps retrying Bridge's AP in the background
  WiFi.softAP(FALLBACK_AP_SSID, FALLBACK_AP_PASS, 1);   // explicit ch=1 — must match ESP-NOW's pin, not left to the default
  esp_wifi_set_channel(1, WIFI_SECOND_CHAN_NONE);       // belt-and-suspenders, same pattern used elsewhere in this sketch
  Serial.printf("[fallback] AP up: SSID=%s IP=%s ch=%d\n", FALLBACK_AP_SSID, WiFi.softAPIP().toString().c_str(), WiFi.channel());
  startBridgeServices();   // mDNS + OTA on the fallback AP itself — the actual point of this page
  fallbackServer.on("/", handleFallbackRoot);
  fallbackServer.on("/ledtest", handleFallbackLedTest);
  fallbackServer.on("/allon", handleFallbackAllOn);
  fallbackServer.on("/alloff", handleFallbackAllOff);
  fallbackServer.on("/retryjoin", handleFallbackRetryJoin);
  fallbackServer.on("/reboot", handleFallbackReboot);
  fallbackServer.on("/resetdefault", handleFallbackReset);
...
void stopFallbackAp() {
  ...
  Serial.println("[fallback] Bridge AP back — fallback AP torn down");
}
...
void updateBridgeJoin() {
  bool staUp = (WiFi.status() == WL_CONNECTED);

  if (staUp && !bridgeStaWasUp) {
    Serial.printf("[bridge-join] connected. IP: %s\n", WiFi.localIP().toString().c_str());
    startBridgeServices();
    if (fallbackApActive) stopFallbackAp();
  } else if (!staUp && bridgeStaWasUp) {
    // Just dropped — give this drop its own fresh timeout window rather than
    // reusing the original boot timestamp (which would trip the fallback
    // threshold immediately on any later drop, long after the real 3 min).
    Serial.println("[bridge-join] connection dropped — starting fresh retry window");
    bridgeJoinStartMs = millis();
  }
  bridgeStaWasUp = staUp;
  ...
  if (!staUp && millis() - bridgeJoinStartMs >= BRIDGE_JOIN_TIMEOUT_MS) {
    Serial.println("[bridge-join] timed out — starting fallback AP");
    startFallbackAp();
  }
}
