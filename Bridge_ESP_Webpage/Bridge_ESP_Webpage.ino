...
// ── Web control page (bench AP — Batch_Provisioning copy only) ───────────────
// Kit-facing control page: Bridge hosts its own AP + web server, EngRoom stays
// ESP-NOW-only. See UTILITY_BATCH_PROVISIONING.md / project_prop_controller_kit.
const char* AP_SSID = "Enterprise D";
const char* AP_PASS = "ncc1701-d";
WebServer   bridgeWebServer(80);
bool        macLoadedFromNvs = false;   // set true in setup() if either peer MAC came from NVS

// ── Bench debug status LED — ESP32-S3 dev board onboard NeoPixel ─────────────
// GPIO48 confirmed on the S3 dev board (see Batch_Bootstrap.ino). NOT present
// on the real Bridge PCB — bench debug aid only, never carry onto real hardware.
#define STATUS_LED_PIN         48
#define STATUS_LED_BRIGHTNESS  40
uint32_t statusFlashUntilMs = 0;

// ── MAC addresses ─────────────────────────────────────────────────────────────
uint8_t broadcastAddress1[] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00}; // EngRoom — run MAC_address_retriver on your board (or use /setmac below)
uint8_t broadcastAddress2[] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00}; // DataPad — run MAC_address_retriver on your board (or use /setmac below)
String  selfMacStr; // Bridge's own MAC, set in setup(), read by /macs
...
#define LED_PAIR_DATAPAD  34   // both ESPs: open a listen-and-adopt window for a broadcasting DataPad (startWifi=4)
#define LED_ALL_OFF       99

// Bridge-local only, kit/no-DataPad firmware — never sent over ESP-NOW, so kept
// well clear of the shared cross-sketch ledCmd range (see control.html CMD map).
#define LED_DEMO_MANUAL   100  // ledValue 1=on 0=off — toggles the autonomous demo cycle, session-only (not saved to NVS)
#define LED_DEMO_SOUND    101  // ledValue 1=on 0=off — gates sound the demo loop itself plays; muted by default
...
// ── Autonomous "showpiece" demo mode (kit/no-DataPad Bridge firmware only) ───
// DEVNOTES.md Pending Work #30. Default-on: once the first auto_ms delay above
// elapses, the model power-cycles itself forever — random EngRoom effects
// while "on," then shuts down, waits, and powers back on. LED_DEMO_MANUAL can
// override it off/back on from the web page, but that override is session-only
// (never written to NVS) — a power-cycle always comes back up demo-on.
bool     demoModeOn       = true;
bool     demoSoundOn      = false;   // LED_DEMO_SOUND — muted by default
bool     demoPoweredPhase = false;   // true once the current "powered on" phase has begun
uint32_t demoPhaseUntilMs = 0;       // millis() when the current on/off phase should end
uint32_t demoNextEffectMs = 0;       // next random-effect trigger time (powered-on phase only)
uint32_t demoWarpOffMs    = 0;       // 0 = no demo-triggered warp effect currently running
#define DEMO_ON_MIN_MS     180000UL  // 3 min
#define DEMO_ON_MAX_MS     360000UL  // 6 min
#define DEMO_OFF_MIN_MS     30000UL  // 30s
#define DEMO_OFF_MAX_MS     90000UL  // 90s
#define DEMO_EFFECT_MIN_MS  15000UL  // 15s between random effects
#define DEMO_EFFECT_MAX_MS  45000UL  // 45s
#define DEMO_WARP_MIN_MS     8000UL  // how long a demo-triggered warp effect runs
#define DEMO_WARP_MAX_MS    20000UL

// ── DataPad auto-adopt pairing (Fix Queue #9 / UTILITY_BATCH_PROVISIONING.md) ─
// LED_PAIR_DATAPAD opens a temporary window; a fresh DataPad broadcasts
// startWifi=4 (ESP-NOW broadcast address, no prior peer knowledge needed on
// either side) to announce itself. Captured in OnDataRecv() below.
bool     pairWindowOpen   = false;
uint32_t pairWindowUntilMs = 0;
#define PAIR_WINDOW_MS 60000UL   // 60s
...
void updateStartup() {
  ...
  seqStep++;
  // Kit config has no DataPad: Bridge is always the sole controller and
  // EngRoom is always physically present, so cascade regardless of
  // startupSolo — that flag only governs Bridge's own local Nav/impulse
  // self-drive (lines above), not whether EngRoom should be told to start.
  if (seqStep == 10) {
...
// ═══════════════════════════════════════════════════════════════════════════════
// AUTONOMOUS DEMO MODE  (kit/no-DataPad firmware only — see globals above)
// ═══════════════════════════════════════════════════════════════════════════════

void relayToEngRoom(int grp, int cmd, int val) {
  struct_message relay;
  memset(&relay, 0, sizeof(relay));
  relay.ledGroup = grp; relay.ledCmd = cmd; relay.ledValue = val;
  esp_now_send(broadcastAddress1, (uint8_t *)&relay, sizeof(relay));
}

void demoFireRandomEffect() {
  if (random(2) == 0) {
    // Fire torpedoes — EngRoom's IDX_PHOTON (Engine_Room_ESP.ino), same group
    // number control.html's "Fire Torpedoes" button already uses.
    relayToEngRoom(10, LED_ON, 0);
#ifdef USE_SOUND
    if (demoSoundOn) soundPlayRandom(SND_WEAPONS_FIRST, SND_WEAPONS_LAST);
#endif
  } else {
    // Engage warp at a random speed for a while, then drop back to normal —
    // EngRoom has no auto-revert for LED_WARP, so track our own timeout.
    relayToEngRoom(0, LED_WARP, 1 + random(10));
    demoWarpOffMs = millis() + random(DEMO_WARP_MIN_MS, DEMO_WARP_MAX_MS + 1);
#ifdef USE_SOUND
    if (demoSoundOn) soundPlayRandom(SND_WARP_FIRST, SND_WARP_LAST);
#endif
  }
}

void updateDemoCycle() {
  if (!demoModeOn) return;
  uint32_t now = millis();

  if (demoWarpOffMs && now >= demoWarpOffMs) {
    demoWarpOffMs = 0;
    relayToEngRoom(0, LED_WARP, 0);
  }

  if (!demoPoweredPhase) {
    // Off phase — includes the very first pre-power-on wait at boot, which
    // reuses the existing auto_ms/autoStartDelayMs NVS setting (see setup())
    // so a buyer's configured startup delay still governs the first cycle.
    if (autoStartDelayMs == 0) return;   // 0 = auto-start disabled, demo cycling stays off too
    if (currentEffect == EFF_IDLE && now >= demoPhaseUntilMs) {
      handleLedCommand(-1, LED_STARTUP, 0);   // Bridge's own updateStartup() cascades to EngRoom
      demoPoweredPhase = true;
      demoPhaseUntilMs = now + random(DEMO_ON_MIN_MS, DEMO_ON_MAX_MS + 1);
      demoNextEffectMs = now + random(DEMO_EFFECT_MIN_MS, DEMO_EFFECT_MAX_MS + 1);
    }
    return;
  }

  // Powered-on phase — wait out the startup/shutdown animation itself before
  // firing effects or ending the phase.
  if (currentEffect != EFF_IDLE) return;
  if (now >= demoPhaseUntilMs) {
    // Mirrors the web page's own Power Off button: relay-only to EngRoom.
    // EngRoom runs its shutdown first and relays LED_SHUTDOWN back to Bridge
    // when done (assembledMode) — see bridgeApplyCmd()'s LED_SHUTDOWN case.
    demoWarpOffMs = 0;
    relayToEngRoom(0, LED_SHUTDOWN, 0);
    demoPoweredPhase = false;
    demoPhaseUntilMs = now + random(DEMO_OFF_MIN_MS, DEMO_OFF_MAX_MS + 1);
    return;
  }
  if (now >= demoNextEffectMs) {
    demoFireRandomEffect();
    demoNextEffectMs = now + random(DEMO_EFFECT_MIN_MS, DEMO_EFFECT_MAX_MS + 1);
  }
}
...
void handleLedCommand(int grp, int cmd, int val) {
  switch (cmd) {
    ...
    case LED_DEMO_MANUAL:
      demoModeOn = (val != 0);
      if (demoModeOn) {
        // Resume from "now" in whichever phase the model is actually in —
        // avoids an unwanted immediate power-on/off if toggled mid-cycle.
        bool poweredNow  = (currentEffect != EFF_IDLE) || engActive || blinkActive;
        demoPoweredPhase = poweredNow;
        demoWarpOffMs    = 0;
        demoPhaseUntilMs = millis() + (poweredNow
                              ? random(DEMO_ON_MIN_MS, DEMO_ON_MAX_MS + 1)
                              : random(DEMO_OFF_MIN_MS, DEMO_OFF_MAX_MS + 1));
        demoNextEffectMs = millis() + random(DEMO_EFFECT_MIN_MS, DEMO_EFFECT_MAX_MS + 1);
      }
      break;
    ...
    case LED_PAIR_DATAPAD:
      pairWindowOpen    = true;
      pairWindowUntilMs = millis() + PAIR_WINDOW_MS;
      Serial.println("[pair] listen-and-adopt window open (60s)");
      break;
...
// ═══════════════════════════════════════════════════════════════════════════════
// BENCH STATUS LED (NeoPixel, S3 dev board only — see STATUS_LED_PIN above)
// ═══════════════════════════════════════════════════════════════════════════════

void statusIdle() {
  neopixelWrite(STATUS_LED_PIN, 0, 0, STATUS_LED_BRIGHTNESS);   // dim blue = AP up, waiting
}

void statusFlash(uint8_t r, uint8_t g, uint8_t b, uint32_t ms = 150) {
  neopixelWrite(STATUS_LED_PIN, r, g, b);
  statusFlashUntilMs = millis() + ms;
}
...
// WEB CONTROL PAGE (bench AP — Batch_Provisioning copy only)
// ═══════════════════════════════════════════════════════════════════════════════
// control.html (see control_html.h) talks to this over a plain GET /cmd?group=
// &cmd=&value= scheme. Group numbers are board-relative, not globally unique
// (e.g. grp=6/10 mean Bridge window G07/G11 in handleLedCommand() above, but
// mean EngRoom's Deflector/Photon groups when they arrive from the web page)
// — so routing is an explicit whitelist of the exact (cmd, group) pairs
// control.html actually sends, not a generic numeric-range split.

void bridgeApplyCmd(int grp, int cmd, int val, bool &local, bool &relay) {
  local = false;
  relay = false;
  switch (cmd) {
    case LED_STARTUP:
      // Bridge's own updateStartup() already cascades LED_STARTUP to EngRoom
      // partway through its sequence (see the esp_now_send to broadcastAddress1
      // in updateStartup()) — relaying here too would just double-trigger it.
      local = true;
      break;

    case LED_SHUTDOWN:
      // Mirrors how DataPad already targets EngRoom for shutdown — EngRoom is
      // the one that relays *to* Bridge once its own shutdown finishes, not
      // the other way around, so this must not also fire locally here.
      relay = true;
      break;
    ...
    case LED_SET_AUTO_MS:
    case LED_SET_CONN_MS:
      // No existing cascade either direction for these — dual-send keeps
      // both boards in sync from a single button press. The four LED_SET_*
      // timing constants were found missing from this routing entirely
      // (silent no-op via /cmd) while bench-testing conn_ms on 2026-09-12 —
      // pre-existing gap, not previously exercised since no control.html
      // button targets them yet.
      local = true;
      relay = true;
      break;

    case LED_WARP:
      // Bridge's handleLedCommand() has no LED_WARP case — only EngRoom
      // drives nacelle warp effects.
      relay = true;
      break;
    ...
    case SND_CMD_VOL:
    case SND_CMD_REPEAT:
      // DY-SV17F sound hardware only exists on Bridge.
      local = true;
      break;
    ...
    case LED_OFF:
    case LED_BLINK:
      if (grp == IDX_NAV || grp == IDX_ENG_A || grp == IDX_ENG_B) {
        local = true;   // Bridge-owned indices (NAV/IM_LEFT/IM_RIGHT)
      } else if (grp == 6 || grp == 10 || grp == 11 || grp == 20) {
        relay = true;   // EngRoom-owned: DEFLECTOR/PHOTON/PHOTON_AFT/NAC_BOTH — see note above
      }
      break;

    case LED_DEMO_MANUAL:
    case LED_DEMO_SOUND:
      // Bridge-local only — EngRoom effects the demo loop fires go out via
      // its own relayToEngRoom() calls, not by relaying this command itself.
      local = true;
      break;

    case LED_PAIR_DATAPAD:
      // Both ESPs need their own listen-and-adopt window open independently —
      // the broadcasting DataPad reaches both directly, no coordination needed.
      local = true;
      relay = true;
      break;

    default:
      // LED_NAV_MODE, LED_FACTORY_RESET: not implemented in firmware /
      // explicitly out of scope this session. Falls through as a logged no-op below.
      break;
  }
}

void handleRoot() {
  bridgeWebServer.send_P(200, "text/html", CONTROL_HTML);
}

void handleCmd() {
  int grp = bridgeWebServer.hasArg("group") ? bridgeWebServer.arg("group").toInt() : 0;
  int cmd = bridgeWebServer.hasArg("cmd")   ? bridgeWebServer.arg("cmd").toInt()   : 0;
  int val = bridgeWebServer.hasArg("value") ? bridgeWebServer.arg("value").toInt() : 0;

  bool local, relay;
  bridgeApplyCmd(grp, cmd, val, local, relay);
  Serial.printf("[/cmd] grp=%d cmd=%d val=%d -> local=%d relay=%d\n", grp, cmd, val, local, relay);

  if (local) {
    handleLedCommand(grp, cmd, val);
    statusFlash(0, STATUS_LED_BRIGHTNESS, 0);   // green
  }
  if (relay) {
    struct_message relayMsg;
    memset(&relayMsg, 0, sizeof(relayMsg));
    relayMsg.ledGroup = grp;
    relayMsg.ledCmd   = cmd;
    relayMsg.ledValue = val;
    esp_err_t res = esp_now_send(broadcastAddress1, (uint8_t *)&relayMsg, sizeof(relayMsg));
    Serial.printf("[/cmd] relay to EngRoom (%02X:%02X:%02X:%02X:%02X:%02X): %s\n",
      broadcastAddress1[0], broadcastAddress1[1], broadcastAddress1[2],
      broadcastAddress1[3], broadcastAddress1[4], broadcastAddress1[5],
      res == ESP_OK ? "queued OK" : "FAILED");
    statusFlash(STATUS_LED_BRIGHTNESS, 0, STATUS_LED_BRIGHTNESS);   // purple — wins over green if both fire
  }

  bridgeWebServer.send(200, "text/plain", (local || relay) ? "ok" : "ok (no-op)");
}

// Bench-only: manually write a peer MAC into NVS to prove the Phase 1 NVS
// read path (UTILITY_BATCH_PROVISIONING.md). GET /setmac?peer=engroom|datapad
// &mac=AA:BB:CC:DD:EE:FF — remove or gate this before any real deployment.
void handleSetMac() {
  String peer  = bridgeWebServer.arg("peer");
  String macStr = bridgeWebServer.arg("mac");

  uint8_t parsed[6];
  if (sscanf(macStr.c_str(), "%hhx:%hhx:%hhx:%hhx:%hhx:%hhx",
             &parsed[0], &parsed[1], &parsed[2], &parsed[3], &parsed[4], &parsed[5]) != 6) {
    bridgeWebServer.send(400, "text/plain", "bad mac format, expected AA:BB:CC:DD:EE:FF");
    return;
  }

  const char *key;
  uint8_t *target;
  if (peer == "engroom")      { key = "mac_engroom"; target = broadcastAddress1; }
  else if (peer == "datapad") { key = "mac_datapad";  target = broadcastAddress2; }
  else {
    bridgeWebServer.send(400, "text/plain", "unknown peer, expected engroom or datapad");
    return;
  }

  prefs.begin("bridge", false);
  prefs.putBytes(key, parsed, 6);
  prefs.end();

  uint8_t oldMac[6];
  memcpy(oldMac, target, 6);
  ...
  Serial.printf("[/setmac] peer=%s new=%s (was %02X:%02X:%02X:%02X:%02X:%02X) — saved to NVS and applied live\n",
    peer.c_str(), macStr.c_str(),
    oldMac[0], oldMac[1], oldMac[2], oldMac[3], oldMac[4], oldMac[5]);
  bridgeWebServer.send(200, "text/plain", "ok — saved to NVS and applied live (reboot also proves persistence)");
}

// GET /macs — Bridge's own MAC plus its currently-configured EngRoom/DataPad
// peer MACs (NVS override if set, hardcoded fallback otherwise). Lets a
// customer recover these without USB/serial access — the boards may already
// be installed in the model by the time someone wants to check/change one.
void handleMacs() {
  char engroomBuf[18], datapadBuf[18];
  snprintf(engroomBuf, sizeof(engroomBuf), "%02X:%02X:%02X:%02X:%02X:%02X",
    broadcastAddress1[0], broadcastAddress1[1], broadcastAddress1[2],
    broadcastAddress1[3], broadcastAddress1[4], broadcastAddress1[5]);
  snprintf(datapadBuf, sizeof(datapadBuf), "%02X:%02X:%02X:%02X:%02X:%02X",
    broadcastAddress2[0], broadcastAddress2[1], broadcastAddress2[2],
    broadcastAddress2[3], broadcastAddress2[4], broadcastAddress2[5]);

  String json = "{\"bridge\":\"" + selfMacStr + "\",\"engroom\":\"" + String(engroomBuf) +
                "\",\"datapad\":\"" + String(datapadBuf) + "\"}";
  bridgeWebServer.send(200, "application/json", json);
}

// GET /status — real EngRoom connectivity (espNowReceived latches true on the
// first ESP-NOW contact ever, connectionLost flips on if none for connLostMs).
// NOT the same thing as "the browser could reach Bridge's own web server" —
// that's trivially always true on a page Bridge itself is serving, which is
// exactly the bug this endpoint replaces (see control.html's old sendCmd()).
void handleStatus() {
  bool online = espNowReceived && !connectionLost;
  bridgeWebServer.send(200, "application/json", String("{\"online\":") + (online ? "true" : "false") + "}");
}
...
// Real signature per the installed Arduino-ESP32 core (esp_now_recv_info_t,
// not a bare MAC pointer) — fixes Fix Queue #11 for this file specifically,
// since DataPad auto-adopt pairing below genuinely depends on the real
// sender MAC (info->src_addr), unlike every other command in this sketch
// which identifies senders by payload content instead.
void OnDataRecv(const esp_now_recv_info_t *info, const uint8_t *incomingData, int len) {
  memcpy(&myData, incomingData, sizeof(myData));
  espNowReceived = true;
  lastContactMs  = millis();

  Serial.printf("[ESP-NOW recv] from=%02X:%02X:%02X:%02X:%02X:%02X grp=%d cmd=%d val=%d\n",
                info->src_addr[0], info->src_addr[1], info->src_addr[2],
                info->src_addr[3], info->src_addr[4], info->src_addr[5],
                myData.ledGroup, myData.ledCmd, myData.ledValue);
  ...
  if (myData.startWifi == 4) {
    // DataPad auto-adopt announce — only act while a pairing window is open,
    // so ordinary bench traffic never accidentally re-adopts a new MAC.
    if (pairWindowOpen) {
      uint8_t newMac[6];
      memcpy(newMac, info->src_addr, 6);
      prefs.begin("bridge", false);
      prefs.putBytes("mac_datapad", newMac, 6);
      prefs.end();
      esp_now_del_peer(broadcastAddress2);
      memcpy(broadcastAddress2, newMac, 6);
      ...
      Serial.printf("[pair] adopted new DataPad MAC %02X:%02X:%02X:%02X:%02X:%02X — saved to NVS, replied with Bridge's identity\n",
        newMac[0], newMac[1], newMac[2], newMac[3], newMac[4], newMac[5]);
    }
  } else if (myData.startWifi == 3) { startWifi1 = 3; saveWifi1 = false; }
...
void setup() {
  Serial.begin(115200);   // bench debug — Bridge has no debug Serial in the real sketch today
  ...
  // Peer MAC storage (Phase 1, UTILITY_BATCH_PROVISIONING.md) — NVS overrides
  // the hardcoded compile-time defaults already sitting in the arrays above.
  // Only accept a stored value when it's exactly 6 bytes; anything else
  // (key absent, or a corrupt/partial write) falls back to the hardcoded MAC.
  if (prefs.getBytesLength("mac_engroom") == 6) {
    prefs.getBytes("mac_engroom", broadcastAddress1, 6);
    macLoadedFromNvs = true;
    Serial.printf("[NVS] EngRoom MAC loaded from NVS: %02X:%02X:%02X:%02X:%02X:%02X\n",
      broadcastAddress1[0], broadcastAddress1[1], broadcastAddress1[2],
      broadcastAddress1[3], broadcastAddress1[4], broadcastAddress1[5]);
  } else {
    Serial.printf("[NVS] EngRoom MAC: NVS empty, using hardcoded fallback %02X:%02X:%02X:%02X:%02X:%02X\n",
      broadcastAddress1[0], broadcastAddress1[1], broadcastAddress1[2],
      broadcastAddress1[3], broadcastAddress1[4], broadcastAddress1[5]);
  }
  if (prefs.getBytesLength("mac_datapad") == 6) {
    prefs.getBytes("mac_datapad", broadcastAddress2, 6);
    macLoadedFromNvs = true;
    Serial.printf("[NVS] DataPad MAC loaded from NVS: %02X:%02X:%02X:%02X:%02X:%02X\n",
      broadcastAddress2[0], broadcastAddress2[1], broadcastAddress2[2],
      broadcastAddress2[3], broadcastAddress2[4], broadcastAddress2[5]);
  } else {
    Serial.printf("[NVS] DataPad MAC: NVS empty, using hardcoded fallback %02X:%02X:%02X:%02X:%02X:%02X\n",
      broadcastAddress2[0], broadcastAddress2[1], broadcastAddress2[2],
      broadcastAddress2[3], broadcastAddress2[4], broadcastAddress2[5]);
  }
  prefs.end();
  ...
  char selfMacBuf[18];
  snprintf(selfMacBuf, sizeof(selfMacBuf), "%02X:%02X:%02X:%02X:%02X:%02X",
           selfMac[0], selfMac[1], selfMac[2], selfMac[3], selfMac[4], selfMac[5]);
  selfMacStr = selfMacBuf;
  Serial.printf("MAC Address: %s\n", selfMacBuf);
  esp_wifi_set_channel(1, WIFI_SECOND_CHAN_NONE);   // set ch=1 before first send — verify on hardware
                                                     // this still pins ESP-NOW's channel with the AP now up
  ...
  // Web control page — routes registered here, dispatch functions defined
  // above in the LED COMMAND DISPATCH section (need handleLedCommand()).
  bridgeWebServer.on("/", handleRoot);
  bridgeWebServer.on("/cmd", handleCmd);
  bridgeWebServer.on("/setmac", handleSetMac);
  bridgeWebServer.on("/macs", handleMacs);
  bridgeWebServer.on("/status", handleStatus);
  bridgeWebServer.begin();

  // Bench status LED: brief white flash if either peer MAC came from NVS
  // this boot, then settle to idle (dim blue = AP up, waiting).
  if (macLoadedFromNvs) {
  ...
  bootMs        = millis();
  lastContactMs = millis();
  demoPhaseUntilMs = bootMs + autoStartDelayMs;   // first demo power-on reuses the configured auto_ms delay
}
...
void loop() {
  ...
  // Skipped entirely once demoModeOn — updateDemoCycle() owns the first power-on
  // too in that case (it reuses this same autoStartDelayMs for the first delay).
  if (!demoModeOn && !autoStartFired && !espNowReceived &&
  ...
  if (pairWindowOpen && millis() >= pairWindowUntilMs) {
  ...
  updateEffects();
  ArduinoOTA.handle();
  bridgeWebServer.handleClient();

  if (statusFlashUntilMs && millis() >= statusFlashUntilMs) {
    statusFlashUntilMs = 0;
    statusIdle();
  }
}
