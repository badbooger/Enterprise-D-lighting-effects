...
**Fuse and reverse-polarity protection:** the external supply connects through a barrel jack instead of the model's battery boxes — unplug all three battery packs (both saucer packs and the neck pack) before running this way, since nothing in this design accounts for a battery pack and an external supply both being connected at once. Add an inline fuse (slow-blow, rated above the model's combined nominal draw — see `HARDWARE.md` for the full LED power budget) and a basic reverse-polarity diode in series with the barrel jack, outside the model where both are easy to inspect and replace. This is bare-minimum protection, not a full power-supply design — there's no BMS or onboard charge/discharge protection anywhere in this build. **Exact fuse rating, diode part, barrel jack size, and wiring routing are left to you** — every build's access points and cable runs differ enough that one prescribed part list won't fit every setup.
